...
übernehmen. Der zunächst geplante externe OpenCode-Console-Zugang für
`mimo-v2.6-flash-free` wurde im Live-Test mit HTTP 403 abgewiesen und darf
nicht als funktionsfähige Plesk-Anbindung ausgegeben werden. Bis ein erlaubter
Anbieterzugang geprüft ist, `LLM_PROVIDER=stub` sichtbar markiert belassen;
[Anbieterbefund und Datenschutzgrenzen](providers.md) beachten. OpenCode Go
bietet die Variante `mimo-v2.6-flash` ohne `-free` mit Tokenpreisen.
